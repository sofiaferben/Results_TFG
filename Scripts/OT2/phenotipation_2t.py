...
def run(protocol: opentrons.protocol_api.ProtocolContext):
    ...
    def inductors_to_plate(tuple_inductors,dicc_inductor,dicc_relation,pip_left,pip_right,plate_dilutions,V_final,protocol):
        """
        Distributes inductor dilutions into final plates
        """
        import re
        for inductor in tuple_inductors: #IPTG1, IPTG2
            
            name_colony=''
            for colony,list_inductors in dicc_relation.items():
                if inductor in list_inductors:
                    name_colony=colony


            protocol.comment('\nPutting dilution %s for colony %s'%(inductor,name_colony))



            name_inductor,stock,n_replicates,concentrations,column_plate,row_plate_dilutions,type_medium,loc_ind_2x,list_medium,plate,column_plate_dilutions=dicc_inductor[inductor]

            if len(concentrations)==8:#use multichannel pipette
                for pip in [pip_left,pip_right]:
                    if pip.channels==8:
                        pipette=pip
                ...
                if pipette.has_tip==False:
                    pick_up(pipette)
                pipette.distribute(V_final/2,plate_dilutions.columns()[column_plate_dilutions],plate.columns()[column_plate:(column_plate+n_replicates)],new_tip='never',disposal_volume=0)
            
            if len(concentrations)!=8: ##use single channel pipette 
                for pip in [pip_left,pip_right]: 
                    if pip.channels==1:
                        pipette=pip
                rows_dest = [row for row in range(row_plate_dilutions, row_plate_dilutions+len(concentrations))]
                if pipette.has_tip==False:
                    pipette.pick_up_tip()
                #print(plate_dilutions.columns()[column_plate_dilutions][rows_dest])
                #print(plate.rows()[rows_dest][column_plate:column_plate+n_replicates])
                for r in rows_dest:
                    pipette.distribute(V_final/2,plate_dilutions.columns()[column_plate_dilutions][r],plate.rows()[r][column_plate:column_plate+n_replicates],new_tip='never',disposal_volume=0)
    ...
    for type_medium,dicc_dilutions in colony_dilution_stocks.items(): #iterate through colonies --> medi
            for inductor, dicc_inductor_name in dicc_dilutions.items():
                for tuple_inductor in dicc_inductor_name.keys():
                    inductors_to_plate(tuple_inductor,dicc_inductor,dicc_relation,dicc_general['pip left'],dicc_general['pip right'],plate_dilutions,dicc_general['Final volume per well (in ul)'],protocol)
    ...
    for type_liq,source_liq,change_tip in liq_index:
        for name,params_colony in dicc_colonies.items():
            ...
            ##continua por aqui
            #----- ALOME PONER A DICC GENERAL PLATE DILUTIONS, PIP_LEFT, PIP_RIGHT
    ...
    protocol.pause('Put colonies to final plates')
